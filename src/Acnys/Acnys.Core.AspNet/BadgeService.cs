...
namespace Acnys.Core.AspNet
{
    ...
    public class BadgeService : IBadgeRegistry
    {
        ...
        {
            RegistrateBadge("version", "Version", () => Task.FromResult(Assembly.GetEntryAssembly()?.GetCustomAttribute<AssemblyInformationalVersionAttribute>().InformationalVersion.Split('+', 2)[0]), ColorScheme.Blue);
        }
